...
class CouponFormatter {
  ...
  formatCoupon(coupon) {
    return {
      ...
      min_purchase_amount: coupon.minPurchaseAmount ? parseFloat(coupon.minPurchaseAmount) : null,
      max_discount_amount: coupon.maxDiscountAmount ? parseFloat(coupon.maxDiscountAmount) : null,
  ...
  formatUserCoupon(userCoupon) {
    const daysRemaining = userCoupon.expireAt
      ? Math.ceil((new Date(userCoupon.expireAt) - new Date()) / (1000 * 60 * 60 * 24))
      : null
